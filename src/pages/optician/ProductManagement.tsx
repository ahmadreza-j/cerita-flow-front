...
const validationSchema = yup.object({
    ...
    purchasePrice: yup.number().min(0, 'قیمت خرید نامعتبر است').required('قیمت خرید الزامی است'),
    sellingPrice: yup.number().min(0, 'قیمت فروش نامعتبر است').required('قیمت فروش الزامی است'),
    quantity: yup.number().min(0, 'تعداد نامعتبر است').required('تعداد الزامی است'),
    minQuantity: yup.number().min(0, 'حداقل موجودی نامعتبر است').required('حداقل موجودی الزامی است')
});
...
const ProductManagement = () => {
    ...
    const createProduct = useMutation(
        (productData: any) => axios.post('/api/products', productData),
        {
            onSuccess: () => {
                ...
                setSnackbar({ open: true, message: 'محصول با موفقیت ایجاد شد', severity: 'success' });
                handleCloseDialog();
            },
            onError: () => {
                setSnackbar({ open: true, message: 'خطا در ایجاد محصول', severity: 'error' });
            }
        }
    );

    const updateProduct = useMutation(
        (productData: any) => axios.put(`/api/products/${productData.id}`, productData),
        {
            onSuccess: () => {
                queryClient.invalidateQueries('products');
                setSnackbar({ open: true, message: 'محصول با موفقیت بروزرسانی شد', severity: 'success' });
                handleCloseDialog();
            },
            onError: () => {
                setSnackbar({ open: true, message: 'خطا در بروزرسانی محصول', severity: 'error' });
            }
        }
    );
    ...
    const deleteProduct = useMutation(
        (productId: number) => axios.delete(`/api/products/${productId}`),
        {
            onSuccess: () => {
                queryClient.invalidateQueries('products');
                setSnackbar({ open: true, message: 'محصول با موفقیت حذف شد', severity: 'success' });
            },
            onError: () => {
                setSnackbar({ open: true, message: 'خطا در حذف محصول', severity: 'error' });
            }
        }
    );
                    ...
                    <TableBody>
                        {products.map((product: Product) => (
                            <TableRow key={product.id}>
                                ...
                                <TableCell>{formatNumber(product.purchasePrice)} ریال</TableCell>
