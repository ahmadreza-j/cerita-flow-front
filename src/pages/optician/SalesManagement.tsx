...
const SalesManagement = () => {
    ...
    const { data: products = [] } = useQuery(
        ['products', searchTerm],
        () => axios.get('/api/products', { params: { search: searchTerm } }).then(res => res.data)
    );
    ...
    const { data: salesStats } = useQuery(
        ...
        () => {
            ...
            return axios.get('/api/sales/daily-stats', {
                params: {
    ...
    const { data: topProducts } = useQuery(
        ...
        () => {
            if (!startDate || !endDate) return null;
            return axios.get('/api/sales/top-products', {
                params: {
                    startDate: startDate.toISOString(),
                    endDate: endDate.toISOString()
                }
    ...
    const createSale = useMutation(
        (saleData: any) => axios.post('/api/sales', saleData),
        {
            onSuccess: () => {
                ...
                setSnackbar({ open: true, message: 'فروش با موفقیت ثبت شد', severity: 'success' });
                setSaleItems([]);
                formik.resetForm();
            },
            onError: () => {
                setSnackbar({ open: true, message: 'خطا در ثبت فروش', severity: 'error' });
            }
        }
    );
                            ...
                            <Grid container spacing={2}>
                                <Grid item xs={12}>
                                    <Autocomplete
                                        ...
                                        onChange={(_, value) => formik.setFieldValue('patientId', value?.id || '')}
                                    />
                                </Grid>
                                <Grid item xs={12}>
                                    <Button
                                                ...
                                                <TableBody>
                                                    {saleItems.map((item) => (
                                                        <TableRow key={item.productId}>
                                                            ...
                                                            <TableCell>{formatNumber(item.unitPrice)} ریال</TableCell>
                                                            <TableCell>
                                                            ...
                                                            <TableCell>
                                                                {formatNumber(item.quantity * item.unitPrice)} ریال
                                                            </TableCell>
                                                            <TableCell>
                                                                <IconButton
                                                                    color="error"
                            ...
                            <Box mt={3}>
                                ...
                                <ResponsiveContainer width="100%" height={200}>
                                    <LineChart data={salesStats}>
                                        ...
                                        <Tooltip />
                                        <Legend />
                                        <Line type="monotone" dataKey="totalAmount" name="مبلغ فروش" stroke="#8884d8" />
                                        <Line type="monotone" dataKey="totalSales" name="تعداد فروش" stroke="#82ca9d" />
                                    </LineChart>
                                </ResponsiveContainer>
                            </Box>
                        )}
                                        ...
                                        <TableBody>
                                            {topProducts.map((product: any) => (
                                                <TableRow key={product.id}>
                                                    ...
                                                    <TableCell>{formatNumber(product.totalAmount)} ریال</TableCell>
                                                </TableRow>
                                            ))}
                                        </TableBody>
                                    </Table>
