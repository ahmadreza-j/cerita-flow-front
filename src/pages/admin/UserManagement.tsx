...
const UserManagement = () => {
    ...
    const { data: users = [], isLoading } = useQuery(
        ['users', selectedTab],
        () => axios.get(`/api/admin/users${selectedTab !== 'all' ? `/${selectedTab}` : ''}`).then(res => res.data)
    );
    ...
    const createUser = useMutation(
        (userData: any) => axios.post('/api/admin/users', userData),
        {
            ...
            onError: () => {
                setSnackbar({ open: true, message: 'خطا در ایجاد کاربر', severity: 'error' });
            }
        }
    );
    ...
    const updateUser = useMutation(
        (userData: any) => axios.put(`/api/admin/users/${userData.id}`, userData),
        {
            onSuccess: () => {
                queryClient.invalidateQueries('users');
                setSnackbar({ open: true, message: 'کاربر با موفقیت بروزرسانی شد', severity: 'success' });
                handleCloseDialog();
            },
            onError: () => {
                setSnackbar({ open: true, message: 'خطا در بروزرسانی کاربر', severity: 'error' });
            }
        }
    );
    ...
    const toggleUserActive = useMutation(
        (userId: number) => axios.patch(`/api/admin/users/${userId}/toggle-active`),
        {
            onSuccess: () => {
                queryClient.invalidateQueries('users');
                setSnackbar({ open: true, message: 'وضعیت کاربر با موفقیت تغییر کرد', severity: 'success' });
            },
            onError: () => {
                setSnackbar({ open: true, message: 'خطا در تغییر وضعیت کاربر', severity: 'error' });
            }
        }
    );
